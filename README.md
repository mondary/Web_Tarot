...
5. Ouvrez **Tirages** ou **Nuances** depuis la navigation supérieure.

## ⚙️ Réglages

La palette globale et les couleurs d'accent sont définies via les variables CSS du bloc `:root` dans `src/website/v8/index.php`.
...
La V8 ne nécessite pas de build : `index.php` sert l'interface et les ressources, tandis que `vault.sqlite` contient les données et illustrations.

## 🧪 Test local

Le site (V8) se lance depuis `src/website/v8/` avec `launch.command` (double-clic
