...
Statut : `2026.08.31` (V9 - decks et affichage)
...
#### Fixed
- **Barre d'affichage sur mobile** : centrée au-dessus du safe-area, icônes seules (labels masqués), plus de débordement ni de chevauchement.
...
#### Changed
- **Neuf de Bâtons** : « Résistance » devient « Tenir bon », formulation plus directe de la persévérance malgré les difficultés.
...
#### Changed
- **Mode apprentissage** : compteurs séparés pour les cartes distinctes étudiées et les bonnes réponses ; la barre suit désormais les cartes étudiées.
- **Mobile** : numéro de version masqué afin d'éviter son chevauchement avec la navigation.
...
#### Changed
- **Overlay mots-clés** : le calque couvre désormais l'image et la légende de la lame ; le mot-clé reste centré et le nom de la carte apparaît en bas, tous deux en jaune.
...
#### Changed
- **Mode apprentissage mobile** : layout compact repensé pour tenir au-dessus du fold — en-tête une ligne (titre + compteur), image 46vw, options empilées 42px, marges réduites ; sur desktop l'image et la question sont côte à côte
- **Icône Apprendre** : l'œil est remplacé par une flamme (étincelle de connaissance), trait cohérent avec les autres pictos
...
#### Fixed
- **Corruption des données Deniers ↔ Épées** : les 14 paires de mineures (As → Roi) avaient leurs mots-clés endroit/envers, affirmations et réponses OUI/NON intégralement échangés entre Deniers et Épées. Correction dans les sources (`*_reponse.md`), le vault v9 (`/cards/*.json` + `app-data.json` recâblé 78/78). Audit complet des 78 lames validé (familles, numéros, recouvrement lexical, synchro app-data)

### [2026.08.11] - 2026-08-14
#### Changed
- **Intercalaires** : glyphes SVG dessinés (Bâtons, Coupes, Épées, Deniers) restaurés dans les intercalaires de familles, comme en V5 ; les Arcanes majeurs gardent le symbole ✦


### [2026.08.10] - 2026-08-13
#### Changed
- **Top-nav** : icônes dessinées à la main de la V5 restaurées pour Tirages (éventail de cartes) et Recherche (loupe) ; nouvelle goutte à double contour dans le même style pour Nuances
